...
class PanguReasoner:
    ...
    def reason_from_chanlun(
        ...
        if feedback and feedback.strip():
            fb = feedback.strip()
            deduction_path.append(f"HUMAN_FEEDBACK {fb}")
            interpretation = f"【人工质疑】{fb} | {interpretation}"
            if any(w in fb for w in ("高估", "偏强", "overestimate")):
                confidence = round(confidence * 0.85, 2)
                deduction_path.append("PENALTY human_skepticism confidence*0.85")
            elif any(w in fb for w in ("低估", "偏弱", "underestimate")):
                confidence = round(min(confidence * 1.05, 0.99), 2)
                deduction_path.append("BOOST human_correction confidence*1.05")
    ...
    def reason_from_artifact(
        ...
        *,
        ...
        """从 chanlun_replay.json 加载 structure_detail 并推理（支持人工反馈）。"""
        data = _load_json(artifact_path)
        structure = data.get("structure_detail") or {}
        if not structure.get("total_strokes"):
            raise ValueError(
                "artifact 缺少 structure_detail；请使用 pangu-inference DAG 重新跑回测，"
                "或换用 workspace/artifacts/chanlun_btc_100k/chanlun_replay.json"
            )
        last_close = structure.get("last_close")
        if last_close is None and structure.get("last_stroke"):
            last_close = structure["last_stroke"].get("end_price")
        if last_close is not None:
            structure = dict(structure)
            structure["last_close"] = last_close
        return self.reason_from_chanlun(
            structure,
            data.get("metrics"),
            data.get("audit"),
            clean_audit,
