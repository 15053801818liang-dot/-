...
class PersistentMemory:
    ...
    def _load(self):
        self.identity = self._read_json(self.identity_file, {"name": "Pangu", "version": "0.10.0", "sovereignty": "inviolable"})
...
class BoneGuard:
    ...
    def assert_identity(self):
        print(f"[{self.identity}] 主权不可侵犯 | v0.10.0 超我")
...
class SuperBrainAgent:
    ...
    def _load_builtin(self):
        ...
        facts = [
            ...
            Term("attribute", ("pangu", "version", "0.10.0")),
        ...
        self.kg.add_relation("pangu", "version", "0.10.0", 1.0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--mcp':
        ...
        print("盘古 v0.10.0 [超我] 已启动 | 16种认知架构 | 4D记忆 | 梦境引擎 | 知识图谱 | MCP桥接")
        ...
        while True:
            try:
                ...
                if inp.lower() == 'help':
                    print("""
命令:
  grandfather(a, _Who)  查询推理   check_consistency  健康报告
  learn rule ...        显式学习     confirm / reject  确认/拒绝
  remember ...          存储事实     search ...         知识图谱搜索
  dream                 触发梦境     skills              列出技能
  reason ... via cot    指定推理方法  exit                退出
                    """)
                    continue
                agent.run(inp)
