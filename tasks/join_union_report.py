...
输入：
  - chanlun_backtest artifact (缠论回测结果)
  - sc_immune_conclusion.json (从持久化加载，或从参数传入)
输出：
  - union_report.json (跨域联合数据结构)
"""
...
def _extract_chanlun_summary(chanlun_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    bi_list = chanlun_data.get("bi", [])
    duan_list = structure.get("duan", chanlun_data.get("duan", []))
    divergences = structure.get("divergences", chanlun_data.get("divergences", []))

    bi_count = (
        len(bi_list)
        if bi_list
        else structure.get("bi_count")
        or metrics.get("strokes_count", 0)
    )
    duan_count = (
        len(duan_list)
        if duan_list
        else metrics.get("duan_count", 0)
    )
    divergence_events = (
        len(divergences)
        if divergences
        else metrics.get("divergence_count", 0)
    )

    return {
        "bi_count": int(bi_count),
        "duan_count": int(duan_count),
...
def _compute_alignment(chanlun: Dict[str, Any], immune: Dict[str, Any]) -> Dict[str, Any]:
    ...
    status = "aligned" if score >= 0.65 else "neutral" if score >= 0.45 else "misaligned"
...
def _load_sc_immune(params: Dict[str, Any], workspace_dir: str) -> Dict[str, Any]:
    sc_params = params.get("sc_immune") or {}
    if sc_params:
        return sc_params
...
class JoinUnionReport(TaskBase):
    def run(self, params, workspace_dir, dag_id, artifacts):
        ...
        chanlun_artifact = bt_art.get("artifact_path") or bt_art.get("payload", {}).get("artifact_path")
        if not chanlun_artifact:
            raise ValueError("Missing chanlun_backtest artifact")

        with open(chanlun_artifact, encoding="utf-8") as f:
        ...
        union_report = {
            ...
            "immune": {
                ...
                "response_up_genes": sc_data.get("response_up_genes", [])[:10],
                "response_down_genes": sc_data.get("response_down_genes", [])[:10],
